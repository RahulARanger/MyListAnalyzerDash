...
from MyListAnalyzerDash.mappings.enums import ViewHeaderEnum, header_menu_id
...
def search_user(default_user_name="", disable_user_job=False, add=False):
    if add:
        clientside_callback(
            ...
            [
                Output(ViewHeaderEnum.askName, "disabled"),
                Output(ViewHeaderEnum.askName, "required")
            ],
            Input(ViewHeaderEnum.is_it_u, "checked")
        )
        return

    nsfw = dmc.Switch(
        label="nsfw", color="red", onLabel="yes", offLabel="no", id=ViewHeaderEnum.ask_for_nsfw, disabled=disable_user_job)
    ...
    name_input = dmc.TextInput(
        value=default_user_name, id=ViewHeaderEnum.askName, placeholder="User Name, please",
        required=True, withAsterisk=True, rightSection=[
            dmc.ActionIcon(dmc.Image(src=ViewHeaderEnum.addImage), id=ViewHeaderEnum.giveName, color="dark", size="sm")
        ], style=dict(flexGrow=1)
    )

    return expanding_layout(
        dmc.Alert(ViewHeaderEnum.searchAlert, color="orange", title="Note", variant="light"),
        dmc.Space(h=10),
...
def search_user_modal(
    ...
    return get_modal(
        ...
        expanding_layout(
            ...
            dmc.Switch(label="For you ?", color="orange", onLabel="yes", offLabel="no", id=ViewHeaderEnum.is_it_u),
